...
% all segments planes for a single force
A1= seg1p10array(1, index4);
B1 = seg1p01array(1, index4);
C1 = -1;
D1 = seg1p00array(1, index4);

A2= seg2p10array(1, index4);
B2 = seg2p01array(1, index4);
C2 = -1;
D2 = seg2p00array(1, index4);

A3= seg3p10array(1, index4);
B3 = seg3p01array(1, index4);
C3 = -1;
D3 = seg3p00array(1, index4);
...
A5= seg5p10array(1, index4);
B5 = seg5p01array(1, index4);
C5 = -1;
D5 = seg5p00array(1, index4);
 
figure
[x, y] = meshgrid(-1:0.1:1); % Generate x and y data
z1 = -1/C1*(A1*x + B1*y + D1); % Solve for z data
p1 = surf(x,y,z1, 'FaceColor', 'r') %Plot the surface
hold on
z2 = -1/C2*(A2*x + B2*y + D2); % Solve for z data
p2 = surf(x,y,z2, 'FaceColor', 'b')
z3 = -1/C3*(A3*x + B3*y + D3); % Solve for z data
p3 = surf(x,y,z3, 'FaceColor', 'g')
z4 = -1/C4*(A4*x + B4*y + D4); % Solve for z data
p4 = surf(x,y,z4, 'FaceColor', 'c')
z5 = -1/C5*(A5*x + B5*y + D5); % Solve for z data
p5 = surf(x,y,z5, 'FaceColor', 'y')

legend([p1,p2,p3, p4, p5],{'Seg 1','Seg 2','Seg 3', 'Seg 4', 'Seg 5'},'Location','NorthWest');
